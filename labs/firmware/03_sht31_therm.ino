...
int sample_rate = 1*1000; //the rate at which the sensor will collect and publish data to the internet
...
void loop() {
    ...
    temperature = therm(globePin); //read the 
  
    //publish command to publish the data to the data stream on particle in an event called "temp" that will stay thermistor:<temperature-value>
    //Particle.publish("temp", "thermistor:"+String(temperature)+" ");
